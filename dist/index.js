        ...
        app.patch("/api/v1/users/:id", async (req, res) => {
            try {
                ...
                if (mongodb_2.ObjectId.isValid(userId)) {
                    dbUserSnapshot = await usersCollection.findOne({ _id: new mongodb_2.ObjectId(userId) });
                }
                if (!dbUserSnapshot) {
                ...
                if (email)
                    updateSet.email = email.trim().toLowerCase();
                if (image !== undefined)
                    updateSet.image = image;
                if (role)
                    updateSet.role = role; // রোল আপডেট
                // ৩. মূল ইউজার আপডেট (একবার কল করা যথেষ্ট)
                const updateResult = await usersCollection.updateOne({ _id: dbUserSnapshot._id }, { $set: updateSet });
                ...
                if (name || email) {
                    ...
                    if (reviewsCollection)
                        await reviewsCollection.updateMany(subCollectionFilter, updatePayload);
                    if (deliveriesCollection)
                        await deliveriesCollection.updateMany(subCollectionFilter, updatePayload);
                    if (cartCollection)
                        await cartCollection.updateMany(subCollectionFilter, updatePayload);
                }
